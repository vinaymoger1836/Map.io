 ...
 * 1. Sovereign Base installations (Airbases, Naval Ports, Army HQs).
...
export function renderWarSimStateToMap(
  ...
  // 2. Render Friendly Entities
  const friendlyEntities = session.entities.filter((e) => e.iso === factionIso && e.status !== 'destroyed');
...
export function removeWarSimLayers(map: MLMap) {
  ...
  const sourceIds = [SRC_MISSILES, SRC_CONTACTS, SRC_ENTITIES, SRC_PATROLS, SRC_BASES];
